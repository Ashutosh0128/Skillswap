...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Create profile automatically via signal, but ensure it exists
...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Review
        fields = [
            'id', 'session', 'reviewer_name', 'reviewee_name',
            'rating', 'comment', 'ai_flagged', 'ai_flag_reason', 'created_at',
        ]
        read_only_fields = ['ai_flagged', 'ai_flag_reason', 'created_at']
...
# AI Serializers
class AISkillMatchSerializer(serializers.Serializer):
    goals = serializers.CharField(max_length=500)
    current_skills = serializers.CharField(max_length=500, required=False, allow_blank=True)
    preferred_schedule = serializers.CharField(max_length=200, required=False, allow_blank=True)
    max_hourly_rate = serializers.DecimalField(max_digits=10, decimal_places=2, required=False, allow_null=True)


class AILearningPathSerializer(serializers.Serializer):
    goal = serializers.CharField(max_length=500)
    current_level = serializers.CharField(max_length=50, default='beginner')